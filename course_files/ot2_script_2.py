# https://protocols.opentrons.com/protocol/441cc8
def get_values(*names):
    import json

    _all_values = json.loads(
        """{"flow_rate_mm":40,"delay_distribute":2,"delay_reagent_prep":4,"clearance_distribute":1,"clearance_reagent_prep":2,"p300m_side":"left","labware_pcr_plate":"nest_96_wellplate_100ul_pcr_full_skirt","labware_200ul_tips":"opentrons_96_filtertiprack_200ul","labware_reservoir":"nest_12_reservoir_15ml"}"""
    )
...
def run(ctx):
    ...
    [
        ...
        delay_distribute,
        delay_reagent_prep,
        clearance_distribute,
        clearance_reagent_prep,
        p300m_side,
        labware_pcr_plate,
        labware_200ul_tips,
        labware_reservoir,
    ...
    p300m = ctx.load_instrument("p300_multi_gen2", p300m_side, tip_racks=tips300)
    ...
    def set_default_clearances(current_pipette, aspirate_setting, dispense_setting):
    ...
    [*pcr_plates] = [
        ctx.load_labware(labware_pcr_plate, str(slot))
        for slot in [1, 2, 3, 6, 9, 11, 8, 5, 4]
    ]
    ...
    for index, column in enumerate(primer_plate.columns()):
        set_default_clearances(p300m, clearance_reagent_prep, clearance_reagent_prep)
        p300m.pick_up_tip()
        ...
        dispense_with_delay(p300m, 120, column[0], delay_seconds=delay_reagent_prep)
        for repeat in range(10):
            mix_with_delay(p300m, 100, column[0], delay_seconds=delay_reagent_prep)
        set_default_clearances(p300m, clearance_distribute, clearance_distribute)
        aspirate_with_delay(p300m, 125, column[0], delay_seconds=delay_reagent_prep)
        slow_tip_withdrawal(p300m, column[0])
        for plate in pcr_plates:
            dispense_with_delay(
                p300m, 14, plate.columns()[index][0], delay_seconds=delay_distribute
            )
